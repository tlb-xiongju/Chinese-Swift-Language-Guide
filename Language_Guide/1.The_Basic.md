...
除在运行时验证例外外，断言和先决条件也是代码中有用的文档形式。不同于上文中讨论到的错误处理，断言和先决条件是用于可恢复或可预测的错误的。因为一个失败了的断言和先决条件代表着程序不可用的状态，没有办法捕获一个失败的断言。不可能从一个无效状态恢复。当一个断言失败时，至少代码中有点一段无效数据———你不知道为什么它是无效的，或是否还有其他无效状态。

使用断言和先决条件，并不能代替以不可能出现无效状态的方式设计你的代码。即便如此，用它们强化数据和状态，使app在因无效状态发生崩溃时更加可预测，有助于使问题更加易于调试。一旦发生无效状态立即停止执行也有助于限制该无效状态引起的损失。

断言与先决条件的不同在于检查的时机：断言仅在调试构建中被检查，先决条件在调试和发布构建中都会被检查。在发布构建中，条件语句中的断言不会被判断。这意味着你可以在调试阶段使用尽可能多的断言，而不会影响到发布之后的应能。

#### 用断言调试
调用Swift标准库中的`assert(_:_:file:line:)`函数以编写断言。向该函数传入一个结果为`true`或`false`的表达式，和一个当表达式结果为`false`时被显示的信息。例如：
```swift
let age = -3
...
调用函数`preconditionFailure(_:file:line:)`来表明发生了错误——例如，如果调用了`swift`语句的`default`分支，但是所有的有效值应该调用其他分支。
