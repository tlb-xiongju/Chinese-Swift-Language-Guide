...
另外，你也可以通过提供`Character`类型注解符，从单个字符串字面量创建一个单独的字符变量或常量：
```swift
...
相反地，`LATIN CAPITAL LETTER A`(`U+0041`，或`"A"`)，用在英文中，与`CYRILLIC CAPITAL LETTER A`(`U+0410`，或`"A"`)，用在俄文中，是不相等的。字符看起来相似，但是拥有不同的语言意义：
```swift
