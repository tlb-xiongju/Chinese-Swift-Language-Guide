...
一旦指定了类型参数，就可以用它类定义函数参数的类型（例如，函数`swapTwoValues(_:_:)`中的参数`a`和`b`），或作为函数的返回类型，或作为函数体中的类型注解。每种情况，当函数被调用时，类型参数会被一个实际类型替换。（上面`swapTwoValues(_:_:)`的例子中，第一次调用函数，`T`被替换为`Int`，第二次调用时，别替换为`String`。）
